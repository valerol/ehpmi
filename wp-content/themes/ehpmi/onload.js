$(document).ready(function(){
    ...
    if (screen.width > 1024){
        ...
        $(window).on('load resize scroll', function() {
            ...
            $('.map:not(.inner)').each(function() {
                var windowTop = $(window).scrollTop();
                var elementTop = $(this).offset().top;
                var margin = windowTop - elementTop;
                $(this).find('.image').css({ 'margin-top': 250 + margin/5 });
                $(this).find('.text').css({ 'margin-top': margin/10 });
                $(this).find('.text').css({ 'margin-bottom': 10 + margin/10 });
            });
        });
    ...
    // latest news carousel init
    $("#latest-carousel .owl-carousel").owlCarousel({
        //loop:true,
        margin:30,
        nav:true,
        responsive:{
